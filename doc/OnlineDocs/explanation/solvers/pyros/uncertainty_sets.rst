...
.. list-table:: Mathematical definitions of PyROS uncertainty sets of dimension :math:`n`.
   ...
   * - :class:`~pyomo.contrib.pyros.uncertainty_sets.CardinalitySet`
     - :math:`\begin{array}{l} q^{0} \in \mathbb{R}^{n}, \\ \hat{q} \in \mathbb{R}_{+}^{n}, \\ \Gamma \in [0, n], \\ \Delta \in \{-1, 0, +1\}^n \end{array}`
     - :math:`\left\{ q \in \mathbb{R}^{n} \middle| \begin{array}{l} \exists\,\xi^+, \xi^- \in [0, 1]^n\,:\\ \quad \,q = q^{0} + \hat{q} \circ (\xi^+ - \xi^-) \\ \quad \displaystyle \sum_{i=1}^{n} (\xi_{i}^+ + \xi_{i}^-) \leq \Gamma \\ \quad \xi_i^+ = 0 \quad\forall\, i : \Delta_i = -1 \\ \quad \xi_i^- = 0 \quad\forall\, i : \Delta_i = +1\end{array} \right\}`
