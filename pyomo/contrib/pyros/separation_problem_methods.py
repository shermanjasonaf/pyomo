...
def group_ss_ineq_constraints_by_priority(separation_data):
    ...
    for con in sep_model.second_stage.coeff_matching_ineq_cons:
        separation_data.separation_priority_order[con.index()] += 0 * priority_tier_gap
...
def solver_call_separation(
    ...
    if is_obj_state_var_independent:
        # beware: for discrete sets, the separation objective
        #         should be state variable dependent,
        #         since then the role of the separation problems then
        #         is simply to solve for the state variables
